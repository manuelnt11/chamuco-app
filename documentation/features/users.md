...
| `avatar`         | UUID (nullable)     | FK → `assets.id`. The user's current avatar. Can be a GCS-hosted photo (`source: 'gcs'`) or an emoji (`source: 'emoji'`). Resolved to a `ResolvedAsset` (with `url`) by `AssetResolverService` before being returned in API responses. Replaced atoms: old asset record is deleted from the `assets` table and, if GCS-hosted, its object is deleted from Cloud Storage and `makePublic` is called on the new object. |
...
| Data                                                                                                    | Who can see it                                                                |
| ------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------- |
| `display_name`, `avatar` (resolved URL), `bio`                                                          | Any user in the same trip or group                                            |
