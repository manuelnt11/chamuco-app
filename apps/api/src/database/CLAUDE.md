...
| `trip_tasks`              | trips             | Trip checklist item — `scope` discriminates `SHARED` (created by organizer/co-organizer, per-participant completion), `PERSONAL` (owned by `created_by`, single `completed_at`), or `ORGANIZER` (organizer/co-organizer only, single shared `completed_at` + `completed_by` recording which organizer completed it) |
| `trip_task_completions`   | trips             | M:M — per-participant completion of a `SHARED` `trip_task` only; composite PK `(task_id, user_id)`; row presence = completed                                                                                                                                                                                        |
...
| `trip_task_scope`          | `SHARED`, `PERSONAL`, `ORGANIZER`                                                                                | `trip_tasks.scope`                           |
...
| 0039      | `0039_add_trip_task_scope.sql`        | `trip_task_scope` enum; added nullable `trip_tasks.scope` column (step 1 of the `owner_id` → `scope` migration)                                                                                                                                                  |
| 0040      | `0040_finalize_trip_task_scope.sql`   | Added `ORGANIZER` task type. Backfilled `scope` from `owner_id`, set NOT NULL, replaced `idx_trip_tasks_trip_id_owner_id`/`trip_tasks_completed_only_when_personal` with `idx_trip_tasks_trip_id_scope`/`trip_tasks_completed_at_not_shared`, dropped `owner_id` |
| 0041      | `0041_add_trip_task_completed_by.sql` | Added nullable `trip_tasks.completed_by` (FK → `users.id`, restrict) + CHECK `trip_tasks_completed_by_organizer_only` (`completed_by` settable only when `scope = 'ORGANIZER'`) — records which organizer completed an ORGANIZER task                            |
