...
- `@chamuco/shared-types` — `TripParticipantStatus`, `TripRole`, `TripStatus`, `TripVisibility`, `InvitationTokenContext` enums used for role/status checks and invitation context
...
- `@/services/trips.types` — `TripResponse`, `TripParticipantResponse`, `PendingTripParticipantResponse`, `MyTripParticipationResponse` response type shapes
...
- `ORGANIZER_ROLES` (const) — array of `TripRole` values (`ORGANIZER`, `CO_ORGANIZER`) used to check organizer privileges
- `TripParticipantsPage` (component) — default export; fetches trip, participation, participants, and pending requests; renders back nav, action buttons (invite response, join, leave), pending panel (organizers only), participant list, and invitation link widget (organizers only)
- `loadData` (function) — inner async function inside `TripParticipantsPage`; orchestrates parallel API calls, populates state, and sets `pageState`; fetches pending participants only when the current user has an organizer role
