...
- `TripTasksPage` (component) — page that fetches trip tasks and splits them into SHARED ("group", `UsersThreeIcon`) and PERSONAL ("my tasks", `UserIcon`) sections with independent empty states; each section has its own add-task form (separate title state and submit handler) — the shared-task form is shown only to organizers/co-organizers, the personal-task form is always shown; delegates completion toggling, renaming (organizer-gated for SHARED, always available for PERSONAL), and deletion per task via `TripTaskItem`
...
- `describe('TripTasksPage', ...)` (const) — test suite covering section split, empty states, shared-input visibility, create (shared and personal inputs), toggle, organizer-gated rename/delete on shared tasks, always-available rename/delete on personal tasks, rename success and error paths, back link, error handling
