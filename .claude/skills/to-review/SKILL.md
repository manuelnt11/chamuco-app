...
## Step 3 — Update affected INVENTORY.md files

For every directory that has added, removed, or renamed files in this branch, check if an `INVENTORY.md` exists in that directory. If it does, update it to reflect the current state of the directory.

To identify affected directories:

```bash
git diff --name-only origin/main...HEAD | xargs -I{} dirname {} | sort -u
```

For each affected directory with an `INVENTORY.md`:

1. Read the current `INVENTORY.md`.
2. Add entries for any new files not yet listed.
3. Remove entries for any deleted files.
4. Update entries for any renamed or substantially changed files.

Do not create new `INVENTORY.md` files — only update existing ones.
